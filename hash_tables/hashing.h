...
// (alternatives would be to check defensively, OR to use an update semantics
// whenever key occurs already) ENSURES pair (key, value) is inserted in table h
...
//  since writeHT does not implement an update semantics, one way to update is
//  to use read to return index and do a "hard update"

// REQUIRES key occurs at most once in table h
// ENSURES returns array position where key is found or -1 otherwise
//         (this allows the function to be used as auxiliary for update /
//         removal) writes value associated with key in *value uses linear
//         probing treating DELETED positions as occupied
